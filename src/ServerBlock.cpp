...
void ServerBlock::setPort(const std::string &port) {
    ...
    if (intport < 1 || intport > 65535)
        throw std::runtime_error(CONFF "Port has to be in range of 1 - 65535");
    if (std::find(_ports.begin(), _ports.end(), intport) != _ports.end())
        throw std::runtime_error(CONFF "Duplicate port");
...
void ServerBlock::setAutoIndex(const std::string &value) {
    if (value != "on" && value != "off")
        throw std::runtime_error(CONFF "Autoindex has to be 'on' or 'off'");
    _autoIndex = (value == "on");
}
...
void ServerBlock::setBodySize(const std::string &value) {
    if (hasValidUnit(value)) {
        ...
        if (intValue < 0)
            throw std::runtime_error(CONFF "Bodysize has to be positive");
        if(unit == 'k' || unit == 'K') 
...
void ServerBlock::setErrorPage(int code, const std::string &path) {
    if (code < 100 || code > 599)
        throw std::runtime_error(CONFF "error_page code is invalid: " + std::to_string(code));
...
int convertToInt(const std::string &word) {
    if (!std::all_of(word.begin(), word.end(), ::isdigit)) {
        throw std::invalid_argument(CONFF "unexpected word: " + word);
    }
    try {
        return stoi(word);
...
void hasForbiddenSymbols(const std::string &word) {
    for (size_t i = 0; i < word.length(); i++) {
        ...
        if (!(std::isalnum(c) || c == '-' || c == '.' || c == '/' || c == '*'))
            throw std::runtime_error(CONFF "forbidden symbols in: " + word);
    }
}
